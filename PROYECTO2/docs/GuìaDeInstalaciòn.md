...
Comprobar que las herramientas principales quedaron instaladas correctamente:uname -r
...
make clean
make
Si la compilación es exitosa, se generará el archivo:
...
Detener el daemon
En la terminal donde se ejecuta el daemon:
...
El módulo de kernel compila correctamente.

El módulo crea el archivo /proc/continfo_pr2_so1_202307705.

El daemon compila y ejecuta correctamente.

Grafana y Valkey se levantan con Docker Compose.

El script spawn_containers.sh crea contenedores correctamente.

El cronjob ejecuta el script automáticamente.

El daemon genera monitor_logs.jsonl.

El daemon guarda métricas en Valkey.

...
docker run -d --rm --name low_1 alpine sh -c "sleep 300"
docker run -d --rm --name low_2 alpine sh -c "sleep 300"
docker run -d --rm --name low_3 alpine sh -c "sleep 300"
...
docker run -d --rm --name highram_1 python:3.12-alpine sh -c "python3 -c 'x = bytearray(300 * 1024 * 1024); import time; time.sleep(300)'"
